...
test("keeps mobile expense modal support", () => {
  ...
  // Modal presentation (fixed bottom-sheet, backdrop, scroll lock) must be
  // scoped to mobile so it cannot leak onto the inline desktop form. The show
  // rule therefore lives inside the mobile media query, after it opens.
  assert.ok(
    cssSource.indexOf("body.expense-modal-open .entry-panel") >
      cssSource.indexOf("@media (max-width: 1099px)"),
    "expected the modal-open entry-panel rule inside the mobile media query",
  );
});
